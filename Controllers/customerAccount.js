  ...
  try {
    ...
    fetch(urlGraphFb, {
      ...
      const fb = async(response) => {
        ...
        let user = new Customer({ name, email, mobile, type, registrationType });
        user.save();
        const logintoken = { id: user._id };
...
export const updateCustomerProfile = async (req, res, next) => {
  ...
  try {
    ...
    if (!customer)
      ...
      const cus = await Customer.findByIdAndUpdate({ _id: id }, { $set: req.body });
...
export const uploadCustomerProfilePhoto = async (req, res, next) => {
  ...
  try {
    ...
    const cus = await Customer.findByIdAndUpdate({ _id: id }, { profilePicture });
    return res.json({
      success: true,
